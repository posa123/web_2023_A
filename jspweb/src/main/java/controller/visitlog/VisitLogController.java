...
public class VisitLogController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 2. 객체화
		VisitDto visitDto = new VisitDto(vwriter, vpwd, vcontent); System.out.println(visitDto);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 4. 응답
		// response.getWriter().print(result); // 응답은 가능한. js가 ArrayList타입 사용이 불가능 [ 문제발생 ]
		response.setContentType("application/json;charset=UTF-8");
	...
	/* 
	  ...
	  - put,delete 함수도 매개변수 전달 가능하도록 설정 [ 서버마다 ]
		...
		<Connector connectionTimeout="20000" maxParameterCount="1000" port="80" protocol="HTTP/1.1" redirectPort="8443" parsBodyMethods="POST,PUT,DELETE" URIEncoding="UTF-8"/>
	 */
