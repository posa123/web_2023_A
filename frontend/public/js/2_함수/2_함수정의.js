	...
	함수	[ 함 : 상자 / 수 : 숫자 ]
		...
		3. 사용 목적 
			 ...
			 3. 메모리 효율성 높일수 있다.
			 	...
			 	- 전역변수 : { } 없는곳에 선언된 메모리는 모든 곳에서 호출 가능
					* 메모리 : 변수/상수/배열/객체 등등
			...
			4. 함수 선언
				...
				function 함수명 ( 인수1 , 인수2 , 인수3){
					**** 코드 정의 ****
					...
					2. 함수명 : 서러 다른 함수간의 식별용 [ *js내 동일한 이름불가 / 키워드(let,const,for 등) 불가 ]
...
function 함수5( ){ return 5;}			 // 5. 인수 없다 / 반환 값 없다. -> 함수종료 역할
function 함수6() { return; }			 // 6. 인수 없다 / 반환 값 = 단순 함수종료 역할
	// *함수가 종료되는 조건 : 1. '}' 끝났을떄 2. return 키워드를 만났을때 [ 아래 코드 이어도 종료! ]
	
// 2. 함수 호출
console.log('안녕!!')				// 1. 미리 정의된(JS 만든사람) log(인수=출력하고싶은문자열)함수
