...
for( let i = 1 ; i<=10 ; i++){
	// 1. sum = sum+i
	// 2. sum += i
	sum += i // (+=) 기존 sum변수에 i를 더한값을 sum변수 에 대입
}
console.log( sum )
/* 예제4 순서도
...
for( let 단 = 2; 단<=9 ; 단++){ //단 s // 첫번째 for 반복횟수 => 8회;
	for( let 곱 = 1 ; 곱<=9 ; 곱++ ){ //곱 s  // 두번째 for 반복횟수 => 72회 [ 상위반복문*현재반복문 ]
