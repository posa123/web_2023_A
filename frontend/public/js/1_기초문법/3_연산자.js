...
console.log( 'data : ' + (데이터2--) ); // 12 에서 1감소 => 12 [ 출력후에 감소 ]
console.log( 'data : ' + 데이터2); // 11
...
let 연령구간 = 나이<=19 ? '청소년' : '성인'
		// 조건 ( true/false 결과 갖는 논리연산 )
console.log( '38의 연령구간 : ' + 연령구간)

	//예제2 [ 조건1 ? 참1 : 조건 ? 참2 : 조건3 ? 참3 : 거짓]
let 점수 = 78
let 등급 = 점수 >= 90 ? 'A등급' :
			점수 >= 80 ? 'B등급' :
		  		점수 >= 70 ? 'C등급' : '탈락';
...
max = max < value4 ? value4 : max // 2. 만약에 max보다 두번째 입력값이 터 크면 max변수에 두번째 값을 저장 / 아니면 그대로
max = max < value5 ? value5 : max // 3. 만약에 max보다 세번째 입력값이 더 크면 교체 / 아니면 그대로
