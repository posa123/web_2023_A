/*
	...
	JS/JAVA : 객체 지향 언어[ OOP : Object Orientrd Programming ]
		
		클래스 = 사전적인 정의 
		객체 = 실제 존재하는 것 / 설계 토대로 메모레이 할당된 것
		
		세상 모든 형태들을 코드화 = 객체
		대량화/자동화/ ===> 객체지향 프로그래밍 적합 하다.
		...
		2. 목적 :  
			1. 형태를 코드화 [ 자동화 , 대량화 ] : 컴퓨터 피로도 없으니까(=전기)
			2. 객체 특징 = JAVA
					...
					- 배열 단점 : 서로 다른 정보유형들을 하나의 배열에 관리 어렵다!!
						ex) 유재석/강호동/신동엽 3명의 회원(정보:아이디,비밀번호,이름,주소 등) 등록
							...
							2방법 : 인덱스 기준 분류
								회원배열 [ ]
							3방법 : 객체유형
								let 회원1 = { 아이디 : 'qwe' , 비밀먼호 : 'qwe' , 이름 : '유재석' 주소 : '안산'}
								let 회원2 = { 아이디 : 'asd' , 비밀먼호 : 'asd' , 이름 : '강호동' 주소 : '성남'}
								let 회원3 = { 아이디 : 'zxc' , 비밀먼호 : 'zxc' , 이름 : '신동엽' 주소 : '수원'}
								
								
			4. 객체 = { }
				-1. 사용목적 : 서로 다른 데이터유형(속성/필드) 들을 한곳에 저장 / 묶음
				-2. 형태
...
console.log( '아이디속성 호출: ' +객체1.아이디 )		// 객체1 의 특정(아이디) 속성내 데이터 호출
console.log( '비밀번호속성 호출 :' +객체1.비밀번호)  // 객체내 속성명 알고 있으면 데이터 호출 가능하다.
	// console. : 내장객체	-> console객체에서 log함수 호출 
	// document : 내장객체 -> doument객체에서 qeuerySeletor() 함수 호출
	// vs
console.log( 배열1[0] )
//3. 객체의 속성내 데이터 변경
...
// !!!!! : JSON -----------> 서로 ( 웹/앱 관련 )언어들 간의 데이터 구조 형태
