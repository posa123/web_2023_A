...
public class Ex1_실행 {
	public static void main(String[] args) {
		...
		// System.out.println( phone.wifi ); // 오류 발생 : 자식 클래스의 멤버 이므로 불가능
		...
		System.out.println( myPhone.color );	// 해당 객체의 부모클래스 멤버 이므로 사용가능
		System.out.println( myPhone.model );	// 해당 객체의 부모클래스 멤버 이므로 사용가능
		...
		myPhone.setWifi( true );	// 오류 발생없는 이유 : 해당 객체는 자식객체 이므로 메소드는 자식클래스의 멤버이므로 
		myPhone.internet();
	}
}