...
public class Board {
	...
	// 1. 필드 : 객체의 데이터를 저장하는 곳
		/*
		 	- 필드 종류	
		 		- 인스턴스필드 : (힙영역) 객체 생성시 객체 안에 저장되어 있는 필드
		 		- 정적필드	: (메소드영역) [static] 프로그램 실행시 우선할당 되고 객체없이 저장되어 있는 필드
		 	- 필드 타입
	...
	// 2. 생성자 : 객체 초기화 할때 사용
		 ...
		 * 
			...
			- 생성자 역할
				...
				- 2. 유효성검사 할때 / 계산식이 필요한 값을 필드에 저장할떄
			...
			- 생성자 종류[ 오버로딩 으로 인해 다양하게 선언 가능 ]
		  	...
		  	- 기타 키워드
		  		...
		  		- this.필드명		: 외부로부터 들어온 매개변수명 과 현재 클래스내 필드명과 같을때 필드 식별할때
		  		- this( )		: 현재클래스의 생성자 호출
		  		- this.메소드명() : 현재클래스의 메소드 호출 
