...
public class 과제5_키오스크_파일처리 {
	...
	public static void main(String[] args) throws IOException {
		...
		while(true) {
			...
			String[] 재고관리 = fileInfo.split("\n");	//System.out.println(" 제품별 분리 : ")
			
			/*------------------------- 제품 정보 출력---------------------------------------*/
			System.out.println("\n\n-------------------- 메뉴 ------------------- ");
			System.out.println("-1:제품등록 0:결제");
				for( int i = 0; i<재고관리.length; i++) { // 2. 재고관리 배열내 정보출력 // 배열 하나씩 출력
					if( !재고관리[i].equals("") ) { // 배열내 데이터가 공백이 아니면 // 제품 존재
						String name = 재고관리[i].split(",")[0]; // 제품명
						int stock = Integer.parseInt(재고관리[i].split(",")[1]); //재고
						int price = Integer.parseInt(재고관리[i].split(",")[2]); //재고
						int basket = Integer.parseInt(재고관리[i].split(",")[3]); //재고
						if( stock == 0 ) { System.out.println( (i+1) + ":" +name+"[재고없음]");} // 재고가 없으면
						else {System.out.println( (i+1) + ":" +name+"["+price+"]");}// 재고가 있으면
			...
			if( ch == -1 ) { // 만약에 -1번을 입력했으면 제품 등록 기능
				...
				// 3. 파일 내보내기 // 1. 파일출력스트림 객체(if밖에) 2.문자열->바이트배열 변환 3. 바이트배열 내보내기	!예외처리
			...
			/*----------------------- 선택한 제품을 바구니 담기 -------------------------------*/
				...
				if( stock > 0 ) {
					stock--; basket++; System.out.println( name+"제품을 담았습니다.");
				}
