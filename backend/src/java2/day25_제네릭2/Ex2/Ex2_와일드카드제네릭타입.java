...
public class Ex2_와일드카드제네릭타입 {
	...
	public static void main(String[] args) {
		...
		// ------- 코스1 : ( 모든지원서(사람,직장인,학생) ) 누구나 등록 가능한 코스
		Course.registerCourse1( new Applicant<Person>(new Person()));
		Course.registerCourse1( new Applicant<Worker>(new Worker()));
		Course.registerCourse1( new Applicant<Student>(new Student()));
		Course.registerCourse1( new Applicant<HighgStudent>(new HighgStudent()));
		Course.registerCourse1( new Applicant<MiddleStudent>(new MiddleStudent()));
		
		// ------- 코스2 : 학생만 신청 가능
			// Applicant < ? extends Student > applicant
			// 지원서 중에 Student 자손객체 만 가능 [ Person , Worker 불가능 ]
		// Course.registerCourse2( new Applicant<Person>(new Person()));
		// Course.registerCourse2( new Applicant<Worker>(new Worker()));
		Course.registerCourse2( new Applicant<Student>(new Student()));
		Course.registerCourse2( new Applicant<HighgStudent>(new HighgStudent()));
		...
		// Course.registerCourse3( new Applicant<HighgStudent>(new HighgStudent()));
